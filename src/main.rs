...
// Chapter 4 handler
fn _respond_with_message(
    message: String,
    _state: &mut HashMap<String, String>,
    writer: &mut BufWriter<TcpStream>,
) -> io::Result<()> {
    // Respond with hello message & client's message
    write!(
        writer,
        "Hello from flotsam! Your message was:\r\n{}",
        message
    )
}

// Chapter 6 handler (word/definition database)
fn process_word_request(
    request: String,
